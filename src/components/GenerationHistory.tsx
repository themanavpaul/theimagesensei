    ...
    <TooltipProvider>
      <div className="w-full max-w-4xl mx-auto mt-8 sm:mt-12 px-2 sm:px-0">
        ...
        <div className="relative w-full">
          <ScrollArea className="w-full overflow-x-auto pb-2 sm:pb-4">
            <div className="flex gap-2 sm:gap-4 px-1 sm:px-0 min-w-0 w-max">
              {images.map((image) => (
                <div
                  key={image.id}
                  className="w-28 xs:w-36 sm:w-48 shrink-0 glass-morphism rounded-lg overflow-hidden cursor-pointer"
                ...
                >
                  ...
                  <div className="p-2 sm:p-3">
                    <div className="flex justify-between items-start mb-1">
                      <p className="text-[10px] xs:text-[11px] sm:text-xs text-white/70 truncate max-w-[80%]">{image.prompt}</p>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button variant="ghost" size="sm" className="w-5 h-5 sm:w-6 sm:h-6 p-0 flex-shrink-0">
                    ...
                    <p className="text-[8px] xs:text-[9px] sm:text-[10px] text-white/50">{formatDate(image.createdAt)}</p>
                  </div>
                </div>
              ))}
            </div>
